...
$relevant_stat_array = ['ftsy_score', 'minutes_played_stat', 'goals_total_stat', 'pen_scored_stat', 'assists_stat', 'dribbles_success_stat', 'shots_total_stat'
                        , 'crosses_total_stat', 'passes_complete_stat', 'key_passes_stat', 'blocks_stat', 'clearances_stat', 'interceptions_stat', 'tackles_stat'
...
FROM rank_ftsy_score_2023 ftsy

INNER JOIN `rank_tackles_stat_2023` tack
    ON ftsy.player_id = tack.player_id

INNER JOIN `rank_shots_total_stat_2023` shot
    ON ftsy.player_id = shot.player_id

INNER JOIN `rank_saves_stat_2023` save
    ON ftsy.player_id = save.player_id

INNER JOIN `rank_pen_saved_stat_2023` pens
    ON ftsy.player_id = pens.player_id

INNER JOIN `rank_pen_scored_stat_2023` pen
    ON ftsy.player_id = pen.player_id
    
INNER JOIN `rank_key_passes_stat_2023` passkey
    ON ftsy.player_id = passkey.player_id

INNER JOIN `rank_passes_complete_stat_2023` pass
    ON ftsy.player_id = pass.player_id

INNER JOIN `rank_minutes_played_stat_2023` minu
    ON ftsy.player_id = minu.player_id

INNER JOIN `rank_interceptions_stat_2023` ints
    ON ftsy.player_id = ints.player_id

INNER JOIN `rank_goals_total_stat_2023` goal
    ON ftsy.player_id = goal.player_id

INNER JOIN `rank_dribbles_success_stat_2023` drb
    ON ftsy.player_id = drb.player_id

INNER JOIN `rank_duels_won_stat_2023` duel
    ON ftsy.player_id = duel.player_id

INNER JOIN `rank_crosses_total_stat_2023` crs
    ON ftsy.player_id = crs.player_id
    
INNER JOIN `rank_clearances_stat_2023` clr
    ON ftsy.player_id = clr.player_id

INNER JOIN `rank_clean_sheet_stat_2023` cs
    ON ftsy.player_id = cs.player_id

INNER JOIN `rank_assists_stat_2023` ass
    ON ftsy.player_id = ass.player_id
    
INNER JOIN `rank_blocks_stat_2023` blk
