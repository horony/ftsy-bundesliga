...
if ($draft_meta['update_lock'] == 0 and $draft_meta['draft_status'] == 'running' and (date("Y-m-d\TH:i:s\Z", strtotime($draft_meta['expire_ts'])) < date("Y-m-d\TH:i:s\Z", strtotime("now")))) {
    ...
    mysqli_query($con, "
        ...
        SET
            player_id = '".$best_available_player['id']."'
            , player_name = '".$best_available_player['display_name']."'
        WHERE 
            pick = '".$draft_meta['current_pick_no']."'
        ");
    ...
    mysqli_query($con, "
        ...
        SET
            ...
            , ftsy_league_id = 1
        WHERE 
            id = '".$best_available_player['id']."'
        ");

    // Update Draft-Meta
    $seconds_to_add = mysqli_query($con, "
        SELECT
            CASE WHEN dof.round >= round_time_change THEN seconds_last_picks ELSE seconds_first_picks END as seconds
        FROM draft_meta dm
    ...
    if ($draft_meta['current_pick_no'] == ($draft_meta['draft_rounds']*$draft_meta['fantasy_players'])) {
        ...
        mysqli_query($con, "
            UPDATE draft_meta
            SET 
                current_round = (SELECT round FROM draft_order_full WHERE pick = '".$draft_meta['current_pick_no']."' + 1)
