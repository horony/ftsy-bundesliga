...
$relevant_stat_array = ['ftsy_score', 'minutes_played_stat', 'goals_made_stat', 'penalties_made_stat', 'assists_made_stat', 'dribble_success_stat', 'shots_total_stat'
                        , 'crosses_total_stat', 'passes_complete_stat', 'passes_key_stat', 'blocks_stat', 'clearances_stat', 'interceptions_stat', 'tackles_stat'
                        , 'duels_won_stat', 'clean_sheet_stat', 'pen_saved_stat', 'saves_stat'];
...
foreach ($relevant_stat_array as &$stat)  {
    ...
    mysqli_query($con, "create temporary table `".$view_name."` as
        ...
        select  base.player_id
                ...
                , CASE  WHEN base.position_short = 'TW' THEN @curRank_TW := @curRank_TW+1 
...
SELECT  ftsy.player_id, ftsy.display_name, ftsy.position_short
...
FROM rank_ftsy_score_2021 ftsy

INNER JOIN `rank_tackles_stat_2021` tack
    ON ftsy.player_id = tack.player_id

INNER JOIN `rank_shots_total_stat_2021` shot
    ON ftsy.player_id = shot.player_id

INNER JOIN `rank_saves_stat_2021` save
    ON ftsy.player_id = save.player_id

INNER JOIN `rank_pen_saved_stat_2021` pens
    ON ftsy.player_id = pens.player_id

INNER JOIN `rank_penalties_made_stat_2021` pen
    ON ftsy.player_id = pen.player_id
    
INNER JOIN `rank_passes_key_stat_2021` passkey
    ON ftsy.player_id = passkey.player_id

INNER JOIN `rank_passes_complete_stat_2021` pass
    ON ftsy.player_id = pass.player_id

INNER JOIN `rank_minutes_played_stat_2021` minu
    ON ftsy.player_id = minu.player_id

INNER JOIN `rank_interceptions_stat_2021` ints
    ON ftsy.player_id = ints.player_id

INNER JOIN `rank_goals_made_stat_2021` goal
    ON ftsy.player_id = goal.player_id

INNER JOIN `rank_dribble_success_stat_2021` drb
    ON ftsy.player_id = drb.player_id

INNER JOIN `rank_duels_won_stat_2021` duel
    ON ftsy.player_id = duel.player_id

INNER JOIN `rank_crosses_total_stat_2021` crs
    ON ftsy.player_id = crs.player_id
    
INNER JOIN `rank_clearances_stat_2021` clr
    ON ftsy.player_id = clr.player_id

INNER JOIN `rank_clean_sheet_stat_2021` cs
    ON ftsy.player_id = cs.player_id

INNER JOIN `rank_assists_made_stat_2021` ass
    ON ftsy.player_id = ass.player_id
    
INNER JOIN `rank_blocks_stat_2021` blk
