...
<div id="headline" class="row">
    <?php 
        ...
        if (!empty($cup_match['cup_round']) && $cup_match['cup_leg'] == 2) {
            ...
            $home_score_leg2 = $cup_match['ftsy_home_score']; 
            $away_score_leg2 = $cup_match['ftsy_away_score'];
            ...
            if ($leg1_match) {
                $home_score_leg1 = $leg1_match['ftsy_home_score']; 
                $away_score_leg1 = $leg1_match['ftsy_away_score']; 
        ...
        if (!empty($cup_match['cup_round'])) {
            ...
            switch ($cup_match['cup_round']) {
                ...
                case 'final':
                    ...
                    break;
                default:
                    $cup_round_transformed = $cup_match['cup_round'];
            }
            ...
            if ($cup_match['cup_round'] != 'final' && !empty($cup_match['cup_leg'])) {
                ...
                switch ($cup_match['cup_leg']) {
                    ...
                    case 2:
                        ...
                        break;
                    default:
                        $cup_leg_transformed = $cup_match['cup_leg']; 
                }
                echo " " . $cup_leg_transformed;
            }
        }
    ...
    <?php
        ...
        foreach ($teams_to_loop as &$loop_value) {
            ...
            if ($akt_spieltag == $clicked_spieltag) {
                        ...
                        SELECT 
                            ...
                            CASE 
                                ...
                                WHEN fix.match_status IN ('1st', '2nd', 'HT') 
                                    THEN COALESCE(akt.ftsy_score, 0) + ((90 - COALESCE(akt.minutes_played_stat, 0)) / 90) * GREATEST(COALESCE(proj.ftsy_score_projected, 0) - 4, 0)
                                ELSE 0 
                            	END AS player_score
                ...
                // Full roster user
                $my_score_valid = mysqli_query($con, " SELECT COUNT(player_id) AS anz_spieler FROM xa7580_db1.ftsy_player_ownership WHERE ".$ftsy_owner_column." = '".$team_id."' AND ".$ftsy_status_column." != 'NONE' 
               	") -> fetch_object() -> anz_spieler;
                ...
                // Full roster opp
                $his_score_valid = mysqli_query($con, " SELECT COUNT(player_id) AS anz_spieler FROM xa7580_db1.ftsy_player_ownership WHERE ".$ftsy_owner_column." = '".$team_id_opp."' AND ".$ftsy_status_column." != 'NONE' 
                ") -> fetch_object() -> anz_spieler;
            ...
            $bilanz = mysqli_query($con, "    
	            SELECT
	            	COALESCE(SUM(
	                    CASE     
	                    	WHEN ftsy_home_id = '".$team_id."' THEN 
