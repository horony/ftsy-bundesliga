...
SET 
  par.waiver_date_1 = (
    SELECT 	CASE 	WHEN DAYOFWEEK(start_dt) = 3 THEN ADDTIME(CONVERT(DATE_ADD(start_dt, INTERVAL 0 DAY), DATETIME), 120000) -- Dienstag -> Dienstag 12:00
  ...
  , par.waiver_date_2 = (
    SELECT  CASE  WHEN DAYOFWEEK(start_dt) = 3 THEN ADDTIME(CONVERT(DATE_ADD(start_dt, INTERVAL 0 DAY), DATETIME), 180000) -- Dienstag -> Dienstag 18:00
