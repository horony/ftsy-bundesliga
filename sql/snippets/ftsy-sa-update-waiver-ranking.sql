	...
	FROM (
		...
		FROM (
			SELECT 	game.user_id
							...
							, tab.rang
			FROM 		users_gamedata game
			INNER JOIN ftsy_tabelle_2020 tab
				ON 	game.user_id = tab.player_id
						AND tab.spieltag = (SELECT max(spieltag) FROM ftsy_tabelle_2020 WHERE season_id = (SELECT season_id FROM parameter))
						AND tab.season_id = (SELECT season_id FROM parameter)
			) tab, (SELECT @curRank := 0) r
	ORDER BY rang DESC
	) tmp 
