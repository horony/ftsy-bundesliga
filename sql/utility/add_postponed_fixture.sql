...
SET   hst.round_name = scr.round_name
...
LEFT JOIN (
    SELECT  hst.1_ftsy_owner_id as `Besitzer2`
...
WHERE   sch.buli_round_name = 18 -- Edit here! <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
